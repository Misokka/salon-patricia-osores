...
export default function HorairesAdmin() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const res = await axios.get(`/admin/horaires`)
      if (res.data.success) {
  ...
  const toggleOnlineBooking = async () => {
    try {
      ...
      const res = await axios.post(`/admin/horaires`, {
        type: 'settings',
  ...
  const handleCreateTimeRange = async (dayOfWeek: number, data: TimeRangeFormData) => {
    try {
      ...
      if (data.id) {
        // Modification
        const res = await axios.patch(`/admin/horaires`, {
          id: data.id,
  ...
  const generateSlots = async (
    ...
    try {
      ...
      const res = await axios.post(`/admin/horaires/generate-slots`, {
        start_date: today,
        end_date: endDate,
        day_of_week: dayOfWeek,
  ...
  const handleDeleteTimeRange = async () => {
    ...
    try {
      setDeletingId(id)
      const res = await axios.delete(`/admin/horaires?id=${id}`)
  ...
  const handleDuplicateDay = async (sourceDayOfWeek: number, targetDayOfWeek: number) => {
    try {
      ...
      for (const range of createdRanges) {
        const genRes = await axios.post(`/admin/horaires/generate-slots`, {
          start_date: today,
          end_date: endDate,
          day_of_week: targetDayOfWeek,
          start_time: range.start_time,
